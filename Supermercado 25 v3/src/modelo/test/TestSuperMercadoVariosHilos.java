...
class TestSuperMercadoVariosHilos {
	...
	void test() {
		...
		List<Future<Cart>> carrritosFuturos = clientesThread.stream().map(cliente -> {
			return executorService.submit(cliente);
		}).collect(Collectors.toList());
		List<Cart> carritosPresente = carrritosFuturos.stream().map(carritofuturo -> {
			try {
				return carritofuturo.get();
